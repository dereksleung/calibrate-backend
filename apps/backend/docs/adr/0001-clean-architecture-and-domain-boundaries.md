...
- **Presentation and `@calibrate/api-contracts`** own HTTP request/response, cookie, header, and other wire-format shapes. Presentation validates and maps those shapes to application inputs and maps application results to responses. Consider `@calibrate/api-contracts` part of the presentation layer.
