...
export function LoginRecoveryPage({ handoff }: { handoff: LoginRecoveryHandoff }) {
  ...
  return <main className="auth-page-background flex min-h-dvh items-center justify-center px-gutter text-on-background">
    <section className="glass-card w-full max-w-[32rem] rounded-4xl p-xl text-center">
      ...
      <p className="mt-md text-sm text-on-surface-variant">Passkey login and email recovery options will be available in the next step of this experience.</p>
      <Button className="mt-xl" type="button" onClick={() => void navigate({ to: "/signup-login" })}>Back to sign up or log in</Button>
    </section>
  </main>;
}
