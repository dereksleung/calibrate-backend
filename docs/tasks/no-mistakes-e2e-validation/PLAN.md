...
- Configure Playwright on the existing `web` project with the Nx generator. Its inferred `web:e2e` target runs Playwright; a separate `web:e2e-runtime` target owns disposable database lifecycle and invokes that target.
- Use Playwright’s `webServer` array to start and health-check the frontend and backend processes.
- Use `http://localhost:3000` as the frontend `baseURL`, and configure the frontend to call `http://localhost:3001/api/v1`; this matches the existing loopback, CORS, and WebAuthn-origin guards.
...
- [ ] Identify the current `web:test:e2e` target as Vitest-based and separate it from the Playwright target that the Nx generator will infer as `web:e2e`.
...
- [ ] Decide how `web:e2e-runtime` will be selected for web and backend changes; the current project graph has no `web` ↔ `backend` dependency.

**Verification:**

- [ ] `npx nx show project web --json`
- [ ] `npx nx show project backend --json`
- [ ] Document `web:dev` at `http://localhost:3000`, `backend:dev` at `http://localhost:3001/health`, the `VITE_API_BASE_URL`, WebAuthn, database, explicit no-op email mode, and required test-only environment variables.
...
**Description:** After approval, use `@nx/playwright:configuration` to add Playwright to the existing `web` project, then adapt the generated configuration to use a `webServer` array. The generated `web:e2e` target runs Playwright directly; Task 3 adds the separate runtime wrapper. Ask for approval before installing any package or browser binary.

**Acceptance criteria:**

- [ ] The generated Playwright configuration exposes an inferred `web:e2e` target.
- [ ] Playwright starts `web:dev` and uses `http://localhost:3000` as `baseURL`.
- [ ] Playwright starts `backend:dev` and waits for `http://localhost:3001/health`.
- [ ] The frontend server receives `VITE_API_BASE_URL=http://localhost:3001/api/v1`; the backend inherits the isolated E2E database and WebAuthn environment supplied by Task 3.
...
export default defineConfig({
  webServer: [
    ...
    {
      name: 'backend',
      command: 'npx nx run backend:dev',
...
- [ ] Confirm the configuration uses the existing `localhost:3000` origin required by the local-development guard.

**Verification:**

- [ ] `npx nx run web:e2e --outputStyle=static -- --list`

**Dependencies:** Task 1

### Task 3: Implement disposable database lifecycle

**Description:** Create `web:e2e-runtime`, a small E2E wrapper that uses the existing Testcontainers Postgres pattern with a random mapped port, applies migrations, invokes `web:e2e`, and always tears down the database. The Playwright configuration owns frontend/backend process lifecycle through its `webServer` configuration.
...
- [ ] Exports the database connection, `WEBAUTHN_RP_ID=localhost`, `WEBAUTHN_ORIGIN=http://localhost:3000`, and `VITE_API_BASE_URL=http://localhost:3001/api/v1` to the Playwright-managed server commands.
- [ ] Adds an explicit E2E no-op email-delivery mode that overrides a credential loaded from `.env` by `dotenvx`, ensuring the backend selects `NoopEmailSender` and cannot contact Brevo.
...
- [ ] Run `npx nx run web:e2e-runtime --outputStyle=static` twice consecutively without port, process, or container collisions.
...
- [ ] `npx nx run web:e2e-runtime --outputStyle=static -- --grep "local development passkey signup"`
...
commands:
  test: >
    ...
    npx nx run web:e2e-runtime --outputStyle=static
...
- [ ] `web:e2e-runtime` is the explicit baseline for the test step.
...
- [ ] States `npx nx run web:e2e-runtime --outputStyle=static` as the target command for relevant web/backend changes.
...
- [ ] A clean no-mistakes worktree runs `npm ci`, then `web:e2e-runtime`.
